...
function store() {
    formTambah.post(route('plp-karakter.store'), {
        onSuccess: () => useToast(),
...
function update() {
    formEdit.patch(route('plp-karakter.update'), {
        onSuccess: () => useToast(),
        onError: (err) => useToast({
            icon: 'error',
            title: err.plp
...
async function hapus(item) {
    ...
    if (cond) {
        router.delete(route('plp-karakter.destroy', item.id), {
            onSuccess: () => useToast()
        })
    }
}
...
<template>
    <Layout judul="PLP Karakter">
        <Button color="success" class="mb-2" data-modal-target="modalTambah"
            data-modal-toggle="modalTambah">Tambah</Button>
        <Table :items="daftar">
            <template #head>
                <Th>PLP</Th>
                <Th>Siswa</Th>
